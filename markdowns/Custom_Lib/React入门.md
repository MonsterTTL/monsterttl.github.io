...
在React Native中是用Flexbox布局来定义布局的,初体验来说跟Android里的LinearLayout很像，比如说他们都有布局方向，
也可以为每个子元素设置权重值。
...
* componentWillMount：组件将要被挂载（但是还没有被渲染），这个阶段不能操作DOM元素但可以操作属性状态；
...
## 函数组件的生命周期
函数组件没有生命周期，只有渲染阶段，所以函数组件没有初始化阶段和销毁阶段，只有运行阶段。
